---
title: "Mestrado - dados 2018"
author: "Gabriela Galvão"
output:
    html_document:
      toc: yes
      toc_float: yes
---
# Título 
Efeito da estrutura de paisagem sobre o potencial de recuperação de áreas degradadas  
...
Dados necessários  
```{r}
traits<-read.csv("traits_2018.csv", h=T, ";", row.names = 1)
comunidade <- read.csv("comunidade2018_na_ordem.csv", h=T, ";", row.names=1)
cmm<-comunidade[,20:89]
```
  
# PERGUNTA 1  
Qual a sindrome funcional que prevalece?  
  
# HIPÓTESE 1  
H1 As áreas submetidas à restauração possuem uma síndrome funcional da comunidade relacionada a espécies ruderais, com poucas especializações e sementes mais leves  
``````{r}
composicao_funcional <- functcomp(x = traits, a = as.matrix(cmm), CWM.type = c("dom"))
paged_table(composicao_funcional)
```
  
# PERGUNTA 2  
Como essas sindromes variam em funcao da condincao da paisagem?  
...
função multifiti  
Pd: Densidade de manchas de floresta (por/hectare); medida de fragmentação da paisagem.  
NDV:  Índice de Vegetação da Diferença Normalizada.  
```{r}
comunidade_multfit <- read.csv("comunidade2018_na_ordem.csv", h=T, ";")
fits_ndv <- multifit(mod="glmer", multief = colnames(comunidade_multfit[4:11]), 
                     ...
                     criterion = "AIC", plot_est = T)


fits_pd <- multifit(mod="glmer", multief = colnames(comunidade_multfit[12:19]), 
                    formula=Riqueza~multief+(1|area), args=c("family=poisson"),data=comunidade_multfit, 
                    criterion = "AIC", plot_est = T)
```

normalizar e resolver problemas traits  
```{r}

traits$h_max<-log(traits$h_max)
traits$h_min<-log(traits$h_min)
traits$leaf_min<-log(traits$leaf_min)
traits$leaf_max<-log(traits$leaf_max)
traits$semente<- log(traits$semente)
traits$nativas<-as.numeric(traits$nativas)
traits$h_max<-as.numeric(traits$h_max)
traits$h_min<-as.numeric(traits$h_min)
traits$leaf_min<-as.numeric(traits$leaf_min)
traits$leaf_max<-as.numeric(traits$leaf_max)
traits$leaf_tipe<- as.numeric(traits$leaf_tipe)
``` 

Funcional
```{r}
div.func <- dbFD(x = traits[,1:5], a = cmm, corr = "lingoes", CWM.type = "all")

FDis <- div.func$FDis
Feve <- div.func$FEve
RaoQ <- div.func$RaoQ

dados <- cbind(comunidade,FDis, Feve, RaoQ)
```

# HIPÓTESE 2  
areas com maior cobertura vegetal possuem  maior diversidade funcional, compreendendo uma sindrome funcional de atributos de plantas relacionados a especies mais especialistas.

```{r}
model1 <- lm(FDis ~ ndv800, data=dados) 
summary(model1)
```
...
```{r}
model2 <- lm(FDis ~ pd140, data=dados) 
summary(model2)
```

Diversidade Filogenética
```{r}
tree <- read.tree("tree2018.tre")
plot(tree, type="fan", show.tip.label = TRUE, 
            show.node.label = F, edge.color = "red", edge.width = 1.5, 
            tip.color = "black", cex = 0.7, label.offset = 2, no.margin = T)
```  

checar nomes  
```{r}
cmm_p <- comunidade[,1:19]
setdiff(colnames(cmm), tree$tip.label)
```
  
```{r include=FALSE}
tema_grafico <- theme(panel.border =element_rect(colour = "black",
                                               fill = NA, size = .5),
...
Áreas (unidade amostral) nas linhas e espécies nas colunas (variável resposta).  
```{r}
comunidade<- read.csv("comunidade2018_na_ordem.csv", h=T, ";", row.names=1)
comunidade_especies <- comunidade[,20:89]
```
  
Áreas (unidade amostral) nas linhas + métricas de paisagem nas coluna (variável preditoras: pd, ndvi) + tipo de intervencao.
```{r}
comunidade_preditoras <- comunidade[,1:19]
```
  
MPD  
quantificar a distância média do parentesco entre 2 espécies em uma comunidade.
```{r}
MPD <- mpd(comunidade_especies, cophenetic(tree), 
           abundance.weighted = TRUE)
MPD<- as.data.frame(MPD)
rownames(MPD) <- rownames(comunidade)
```
ÁREA pousio1  abriga a composição de espécies mais aparentada (i.e. menor diversidade filogenética) com a distância média entre as espécies de 41.84384 milhões de anos. Cmm menos aparentada =3019 com 199.56851 milhoes de anos
  
MNTD  
quantificar a média de VALORES MÍNIMOS de parentesco entre 2 espécies em uma comunidade. qual o valor médio da distância para o vizinho mais próximo.   
```{r}
MNTD <- mntd(comunidade_especies, cophenetic(tree), 
             abundance.weighted = TRUE)
MNTD<- as.data.frame(MNTD)
rownames(MNTD) <- rownames(comunidade)
```
espécies terminais sao mais aparentadas na area 3018 e menos aparentadas no controle;
(maior div. filo no controle como esperado(?))
```{r}
metricas_divergencia <- data.frame(MPD = MPD,
                                   MNTD = MNTD)
metricas_divergencia <- cbind(metricas_divergencia, comunidade_preditoras)  
```
# HIPÓTESE 4  
Áreas com maior cobertura vegetal possuem maior diversidade filogenética no banco de sementes.  
...
MPD PLOT
```{r}
MPD_plot_ndv <- ggplot(metricas_divergencia, aes(ndv800, MPD)) +
  labs(x = "ndv800",
       y = "Mean Pairwise Distance") +
  geom_point(size = 5, shape = 19, col = "gray") + tema_grafico+geom_smooth(method = lm, se = FALSE, color = "black")
MPD_plot_ndv

MPD_plot_pd <- ggplot(metricas_divergencia, aes(pd140, MPD)) +
  labs(x = "pd140",
       y = "Mean Pairwise Distance") +
  geom_point(size = 5, shape = 19, col = "gray") + tema_grafico+ geom_smooth(method = lm, se = FALSE, color = "black")
MPD_plot_pd
...
MNTD PLOT  
```{r}
MNTD_plot_ndv <- ggplot(metricas_divergencia, aes(ndv800, MNTD)) +
  labs(x = "ndv800", 
...
MNTD_plot_ndv
MNTD_plot_pd <- ggplot(metricas_divergencia, aes(pd140, MNTD)) +
  labs(x = "pd140", 
       y = "Mean Nearest Taxon Distance", 
       size = 8)+
  geom_point(size = 5, shape = 19, col = "gray") + 
...
# REPETIR TUDO SEM ESPÉCIES EXÓTICAS

Dados sem exótica  
```{r}
traits_SE<-read.csv("traits_2018_sem_exotica.csv", h=T, ";", row.names = 1)
comunidade_SE <- read.csv("comunidade2018_sem_exotica.csv", h=T, ";", row.names=1)
cmm_SE<-comunidade_SE[,20:75]
```
...
``````{r}
composicao_funcional_SE <- functcomp(x = traits_SE, a = as.matrix(cmm_SE), CWM.type = c("dom"))
paged_table(composicao_funcional_SE)
```
## PERGUNTA 2  
Como essas sindromes variam em funcao da condincao da paisagem?  
...
NDV:  Índice de Vegetação da Diferença Normalizada.  
```{r}
comunidade_multfit_SE <- read.csv("comunidade2018_sem_exotica.csv", h=T, ";")
fits_ndv_SE <- multifit(mod="glmer", multief = colnames(comunidade_multfit_SE[4:11]), 
                     formula=Riqueza~multief+(1|area), args=c("family=poisson"),data=comunidade_multfit_SE, 
                     criterion = "AIC", plot_est = T)


fits_pd_SE <- multifit(mod="glmer", multief = colnames(comunidade_multfit_SE[12:19]), 
                    formula=Riqueza~multief+(1|area), args=c("family=poisson"),data=comunidade_multfit_SE, 
                    criterion = "AIC", plot_est = T)
```

```{r include=FALSE}
traits_SE$h_max<-log(traits_SE$h_max)
traits_SE$h_min<-log(traits_SE$h_min)
traits_SE$leaf_min<-log(traits_SE$leaf_min)
traits_SE$leaf_max<-log(traits_SE$leaf_max)
traits_SE$semente<- log(traits_SE$semente)
traits_SE$nativas<-as.numeric(traits_SE$nativas)
traits_SE$h_max<-as.numeric(traits_SE$h_max)
traits_SE$h_min<-as.numeric(traits_SE$h_min)
traits_SE$leaf_min<-as.numeric(traits_SE$leaf_min)
traits_SE$leaf_max<-as.numeric(traits_SE$leaf_max)
traits_SE$leaf_tipe<- as.numeric(traits_SE$leaf_tipe)
``` 

funcional  
```{r}
div.func_SE <- dbFD(x = traits_SE[,1:5], a = cmm_SE, corr = "lingoes", CWM.type = "all")
```
  
```{r include=FALSE}
FDis_SE <- div.func_SE$FDis
Feve_SE <- div.func_SE$FEve
RaoQ_SE <- div.func_SE$RaoQ
dados_SE <- cbind(comunidade_SE,FDis_SE, Feve_SE, RaoQ_SE)
```  
...
```{r}
model1_SE <- lm(FDis_SE ~ ndv800, data=dados_SE) 
summary(model1_SE)
```
...
Áreas com maior densidade de manchas possuem menor diversidade funcional, compreendendo uma síndrome funcional de atributos de plantas relacionados à espécies mais generalistas.
```{r}
model2_SE <- lm(FDis_SE ~ pd140, data=dados_SE) 
summary(model2_SE)
```
...
```{r}
tree <- read.tree("tree2018.tre")
tree_SE <- drop.tip(tree, c("Nicotiana_glauca", "Tridax_procumbens
", "Bidens_pilosa", "Boerhavia_diffusa", "Trianthema_portulacastrom", "
Portulaca_oleracea", "Amaranthus_viridis", "Dactyloctenium_aegyptium", "Eragrostis_pilosa", "Eragrostis_cilianensis", "Eragrostis_tenella", "Eragrostis_ciliaris", "Digitaria_nuda", "Aristida_adscensionis"))

plot(tree_SE, type="fan", show.tip.label = TRUE, 
            show.node.label = F, edge.color = "red", edge.width = 1.5, 
            tip.color = "black", cex = 0.7, label.offset = 2, no.margin = T)
```  
...
checar nomes  
```{r}
cmm_p <- comunidade_SE[,1:19]
setdiff(colnames(cmm_SE), tree_SE$tip.label)
```
...
comunidade_SE<- read.csv("comunidade2018_sem_exotica.csv", h=T, ";", row.names=1)
names(comunidade_SE)
comunidade_especies_SE <- comunidade_SE[,20:75]
```
  
Áreas (unidade amostral) nas linhas + métricas de paisagem nas coluna (variável preditoras: pd, ndvi) + tipo de intervencao.
```{r}
comunidade_preditoras_SE <- comunidade_SE[,1:19]
```
  
MPD  
quantificar a distância média do parentesco entre 2 espécies em uma comunidade.
```{r}
MPD_SE <- mpd(comunidade_especies_SE, cophenetic(tree_SE), 
           abundance.weighted = TRUE)
MPD_SE<- as.data.frame(MPD_SE)
rownames(MPD_SE) <- rownames(comunidade_SE)
MPD_SE
```
ÁREA pousio1  abriga a composição de espécies mais aparentada (i.e. menor diversidade filogenética) com a distância média entre as espécies de 41.84384 milhões de anos. Cmm menos aparentada =3019 com 199.56851 milhoes de anos
...
quantificar a média de VALORES MÍNIMOS de parentesco entre 2 espécies em uma comunidade. qual o valor médio da distância para o vizinho mais próximo.   
```{r}
MNTD_SE <- mntd(comunidade_especies_SE, cophenetic(tree_SE), 
             abundance.weighted = TRUE)
MNTD_SE<- as.data.frame(MNTD_SE)
rownames(MNTD_SE) <- rownames(comunidade_SE)
MNTD_SE
```
espécies terminais sao mais aparentadas na area 3018 e menos aparentadas no controle;
(maior div. filo no controle como esperado(?))
```{r}
metricas_divergencia_SE <- data.frame(MPD_SE = MPD_SE,
                                   MNTD_SE = MNTD_SE)
metricas_divergencia_SE <- cbind(metricas_divergencia_SE, comunidade_preditoras_SE)  
```
## HIPÓTESE 4  
Áreas com maior cobertura vegetal possuem maior diversidade filogenética no banco de sementes.  
...
MPD PLOT
```{r}
MPD_plot_ndv_SE <- ggplot(metricas_divergencia_SE, aes(ndv800, MPD_SE)) +
  labs(x = "ndv800",
       y = "Mean Pairwise Distance") +
  geom_point(size = 5, shape = 19, col = "gray") + tema_grafico+geom_smooth(method = lm, se = FALSE, color = "black")
MPD_plot_ndv_SE

MPD_plot_pd_SE <- ggplot(metricas_divergencia_SE, aes(pd140, MPD_SE)) +
  labs(x = "pd140",
       y = "Mean Pairwise Distance") +
  geom_point(size = 5, shape = 19, col = "gray") + tema_grafico+ geom_smooth(method = lm, se = FALSE, color = "black")
MPD_plot_pd_SE
...
MNTD PLOT  
```{r}
MNTD_plot_ndv_SE <- ggplot(metricas_divergencia_SE, aes(ndv800, MNTD_SE)) +
  labs(x = "ndv800", 
       y = "Mean Nearest Taxon Distance", 
       size = 8)+
  geom_point(size = 5, shape = 19, col = "gray") + 
...
MNTD_plot_pd_SE <- ggplot(metricas_divergencia_SE, aes(pd140, MNTD_SE)) +
  labs(x = "pd140", 
       y = "Mean Nearest Taxon Distance", 
       size = 8)+
  geom_point(size = 5, shape = 19, col = "gray") + 
